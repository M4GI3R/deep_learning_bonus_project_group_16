"""Reusable inference for repository training and local evaluation."""
...
def forecast(
    ...
    """Generate one direct prediction for every requested series/timestamp row."""
    ...
    for series_id, requested in requested_features.groupby("series_id", sort=False):
        ...
        if actual_length < prediction_length:
            future = np.concatenate(
                [
                    future,
                    np.repeat(future[-1:], prediction_length - actual_length, axis=0),
                ]
            )
        prepared.append(
            {
    ...
    with torch.inference_mode():
        for start in range(0, len(prepared), inference_batch_size):
            ...
            for item, normalized in zip(batch, normalized_batch, strict=True):
                ...
                predictions = normalized[:actual_length] * item["scale"] + item["mean"]
    ...
    result = forecast_index[keys].merge(
        unordered, on=keys, how="left", validate="one_to_one"
    )
    if len(result) != len(forecast_index) or not np.isfinite(
        result["prediction"]
    ).all():
        raise ValueError(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate forecasts from a checkpoint.")
