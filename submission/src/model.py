...
class DLinear(nn.Module):
    """DLinear/NLinear hybrid with a separate known-future covariate head.

    The target path follows the paper closely: trend and seasonal components are
    projected independently along the temporal axis.  Last-value centering adds
    the distribution-shift robustness of NLinear.  Exogenous variables are kept
    out of the decomposition and enter through a small point-wise future head.
    """
    ...
    def __init__(
        ...
        # The official DLinear initialization starts as a stable temporal mean.
        ...
        # Start from the robust linear target forecast and learn exogenous
        # corrections rather than letting random covariate weights dominate it.
    ...
    def forward(
        ...
        *,
        ...
        del history_features  # Historical target dynamics are modeled separately.
        if future_features is None:
            raise ValueError("DLinear requires known future features")

        # NLinear-style centering protects the temporal projections against a
        # changing level between training and the forecast origin.
        level = x[:, -1:]
        centered = x - level
...
class CausalBlock(nn.Module):
    """Paper-style residual block with two dilated causal convolutions."""
    ...
    def __init__(
        ...
        # Drop complete channels, matching the spatial dropout used in the paper.
...
class TCN(nn.Module):
    """Direct sequence-to-sequence TCN over history and known future inputs.

    Future target positions contain a seasonal placeholder, never ground-truth
    targets.  The causal TCN therefore predicts the complete horizon in one pass
    without autoregressive exposure bias.
    """
    ...
    def __init__(self, config: dict) -> None:
        ...
        # target/seasonal-placeholder + covariates + future-position indicator
